...
export function expedienteAClienteDetalle(e: ExpedienteKYC): ClienteDetalle {
  ...
  return {
    ...
    nacionalidad: esNatural
      ? pn?.nacionalidad ?? null
      : pj?.pais_constitucion ?? null,
...
export interface ParametroRiesgo {
  id: string
  factor: string
  descripcion: string
  peso: number
...
export const ClientesRiesgoService = {
  factores: (id: string) =>
    fetchJson<RiesgoResult>(`/api/v1/clientes/${id}/factores-riesgo`),
  overrideRiesgo: (
    id: string,
    nivel_riesgo_override: string,
    justificacion_override: string,
  ) =>
    fetchJson<ExpedienteKYC>(`/api/v1/clientes/${id}/override-riesgo`, {
      method: "PATCH",
      body: JSON.stringify({ nivel_riesgo_override, justificacion_override }),
    }),
}

export const ParametrosService = {
  listar: () => fetchJson<ParametroRiesgo[]>("/api/v1/parametros/riesgo"),
  actualizar: (id: string, peso: number, activo?: boolean) =>
    fetchJson<ParametroRiesgo>(`/api/v1/parametros/riesgo/${id}`, {
      method: "PATCH",
      body: JSON.stringify({ peso, ...(activo !== undefined ? { activo } : {}) }),
    }),
}
